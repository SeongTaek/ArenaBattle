...
AABSection::AABSection()
{
	...
	{
		ABLOG(Error, TEXT("Failed to load staticmesh asst. : %s"), *GateAssetPath);
	}

	static FName GateSockets[] = { { TEXT("+XGate") },
		{ TEXT("-XGate") },
		{ TEXT("+YGate") },
		{ TEXT("-YGate") } };
...
void AABSection::OnKeyNPCDestroyed(AActor* DestroyedActor)
{
	...
	auto AGameMode = Cast<AABGameMode>(GetWorld()->GetAuthGameMode());
	ABCHECK(nullptr != AGameMode);
	AGameMode->AddScore(ABPlayerController);
